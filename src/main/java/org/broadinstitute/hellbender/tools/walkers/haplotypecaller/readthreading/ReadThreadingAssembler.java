...
public final class ReadThreadingAssembler {
     ...
     * @param swParameters              {@link SWParameters} used to align dangling ends in assembly graphs to the reference sequence
    ...
    public AssemblyResultSet runLocalAssembly(final AssemblyRegion assemblyRegion,
        ...
        if (generateSeqGraph) {
            assembleKmerGraphsAndHaplotypeCall(refHaplotype, refLoc, header, aligner, swParameters,
                    correctedReads, nonRefSeqGraphs, resultSet, activeRegionExtendedLocation);
        } else {
            assembleGraphsAndExpandKmersGivenHaplotypes(refHaplotype, refLoc, header, aligner, swParameters,
    ...
    private void assembleKmerGraphsAndHaplotypeCall(final Haplotype refHaplotype, final SimpleInterval refLoc, final SAMFileHeader header,
                                                    final SmithWatermanAligner aligner, final SWParameters swParameters, final List<GATKRead> correctedReads,
        ...
        findBestPaths(nonRefSeqGraphs, assemblyResultBySeqGraph, refHaplotype, refLoc, activeRegionExtendedLocation, resultSet, aligner, swParameters);
    }
            ...
            if (!hasAdequatelyAssembledGraph) {
                ...
                if (assembledResult != null && assembledResult.getStatus() == AssemblyResult.Status.ASSEMBLED_SOME_VARIATION) {
                    ...
                    findBestPaths(Collections.singletonList(graph), Collections.singletonMap(graph, assembledResult),
                            refHaplotype, refLoc, activeRegionExtendedLocation, null, aligner, swParameters);
     ...
     * @param aligner               Smith-Waterman aligner to use for aligning the discovered haplotype to the reference haplotype
     * @param swParameters          Smith-Waterman parameters to use for aligning the discovered haplotype to the reference haplotype
    ...
    List<Haplotype> findBestPaths(final Collection<T> graphs, final Map<T, AssemblyResult> assemblyResultByGraph,
                                  ...
                                  final AssemblyResultSet resultSet, final SmithWatermanAligner aligner, final SWParameters swParameters) {
        ...
        for( final BaseGraph<V, E> graph : graphs ) {
            ...
            for (final KBestHaplotype<V, E> kBestHaplotype :
                ...
                if (!returnHaplotypes.contains(h)) {
                    ...
                    final Cigar cigar = CigarUtils.calculateCigar(refHaplotype.getBases(), h.getBases(), aligner, swParameters, SWOverhangStrategy.SOFTCLIP);
                    ...
                    if (cigar == null) {
                        ...
                        final Cigar cigarWithIndelStrategy = CigarUtils.calculateCigar(refHaplotype.getBases(), h.getBases(), aligner, swParameters, SWOverhangStrategy.INDEL);
